...
//		String words[] = document.split("\\s+");
    ...
    public static double getShortDist(String document, String word1, String word2){
        document = document.toLowerCase();
        word1 = word1.toLowerCase();
        word2 = word2.toLowerCase();
        ...
        for(int i = 0; i < words.length; i++){
            ...
            if(wordMap.containsKey(words[i])){
                arrList  = wordMap.get(words[i]);
            }
            arrList.add(i);
            wordMap.put(words[i], arrList);
        }

        int start = wordMap.get(word1).stream().min(Integer::compare).orElse(0);
        int end = wordMap.get(word2).stream()
                .filter(x -> x > start)
                .min(Integer::compare)
                .orElse(words.length - 1);
    ...
    public static boolean pass() {
        ...
        return getShortDist(document, "and", "graphic") == 6d && getShortDist(document, "transfer", "it") == 14d
                && getShortDist(document, "Design", "filler") == 25d;
    }
    ...
    static {
        ...
        sb.append(
                "In publishing and graphic design, lorem ipsum is a filler text commonly used to demonstrate the graphic elements and");
        sb.append(
